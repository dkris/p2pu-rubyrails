...
The folks at "EdgeCase":http://edgecase.com/ created a series of partially-completed Ruby scripts called "Ruby Koans":http://rubykoans.com/.

They also write automated unit tests for these scripts, all of which fail when you first download them.  Your job is to modify the source files (not the tests!) to make all the tests pass.

Along the way, you will learn Ruby syntax and culture.
...
This means that you fixed the *test_assert_truth* method, and the test you must fix is *test_assert_with_message*, also from *about_asserts.rb*.

Your task this week is to complete all 274 of the koans.

As always, please use the Google Group and the IRC channel for assistance.  If you help another participant, try to make sure he or she fully understands the concepts, rather than simply providing the solution with no other context.
