...
Install JRuby using the installer available on "the JRuby site":http://jruby.org/download.  Make sure to download the x64 version if you are running a 64-bit version of Windows; if not, download the plain Windows Executable.
...
{% highlight bash %}
apt-get install build-essential bison openssl libreadline6 libreadline6-dev curl git-core zlib1g zlib1g-dev libssl-dev libyaml-dev libsqlite3-0 libsqlite3-dev sqlite3 libxml2-dev autoconf
{% endhighlight %}
